...
function Pages() {
  ...
  let setPage = (index) => {
    if (canvasManager) canvasManager.off("history:changed");
    setActivePage({ index: index });
  };
  useEffect(() => {
    if (canvasManager && template) {
      const page = template.pages[activePage.index];
      if (!page && canvasManager) {
        canvasManager.off("history:changed");
        canvasManager.clear();
        return;
      }
      canvasManager.importFromJSON(page);
      canvasManager.on("history:changed", () => {
        template.pages[activePage.index] = canvasManager.exportToJSON();
        setTemplate(Object.assign({}, template));
      });
    }
    return () => {
